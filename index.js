...
// // 1.2) You are challenged to find the 4th element of the array, and you came up with this solution: #3, bc its the 4th element in the array
...
function squareEveryDigit(number) {
  ...
  let realDigits = newNum.map(Number);
  let sqrNum = [];

  for (i = 0; i < realDigits.length; i++) {
    if (i > -1) {
      sqrNum.push(realDigits[i] * realDigits[i]);
    }
  }
  return sqrNum.join("");
}
